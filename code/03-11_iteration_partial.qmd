...
nematode_w <- nematode %>%
  ...
  mutate(rndvi = ndvi(nir, r),
         gndvi = ndvi(nir, g),
         bndvi = ndvi(nir, b),
         rendvi = ndvi(nir, re)) %>% # called ndre
...
meancv2 = function(df, col) {
  # Calculate intermediary steps of mean and sd
  mean = mean({{df}} %>% pull({{col}}), na.rm = T) # curly brackets to signify dataframe
...
for (i in 3:ncol(nematode_w)) {
  ...
  # Print a message to keep track of where in the loop at eaach iteration
...
nematode_mods = nematode_w %>%
  # Pivoting longer for the response variables
  pivot_longer(cols = mel_s:eggs_r,
               names_to = "resp_var",
               values_to = "resp_val") %>%

  # Pivoting longer for the explanatory variables  
